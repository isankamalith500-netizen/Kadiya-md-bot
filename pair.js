    ...
    // ════════════ MENU ════════════
        ...
        case 'menu':
                case 'list': {
                    ...
                    menuText += `┊ 👤 𝓝𝓪𝓶𝗲 : ${pushname}\n┊ ⚡ 𝓤𝓹𝓽𝓲𝓶𝗲 : ${getUptime()}\n└──⟡ ━━━━━━━━━━━━━━━━ ⟡\n\n`;
                    menuText += `Click the button below to view categories.`;
                    ...
                    const buttons = [
                        {
                            ...
                            "buttonParamsJson": JSON.stringify({
                                ...
                                "sections": [
                                    {
                                        "title": "KADIIYA MINI COMMANDS",
                                        "rows": Object.keys(CATEGORY_MAP).map(num => ({
                                            "title": `${CATEGORY_MAP[num].emoji} ${CATEGORY_MAP[num].title}`,
                                            "description": `View ${CATEGORY_MAP[num].title}`,
                                            "id": `action_cat_${num}`
                                        }))
                                    }
                                ]
                        ...
                        },
                        {
                            ...
                            "buttonParamsJson": JSON.stringify({
                                ...
                                "url": "https://whatsapp.com/channel/0029Vb69K9665yDEFt3DRR0D",
                                "merchant_url": "https://whatsapp.com/channel/0029Vb69K9665yDEFt3DRR0D"
                            })
                        }
                    ];
                            ...
                            message: {
                                interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                    ...
                                    footer: proto.Message.InteractiveMessage.Footer.fromObject({
                                        text: "𝗔έsthatic 𝗤uέέn 𝗕y 𝗖hamod 𝜗𝜚⋆"
                                    }),
                                    header: proto.Message.InteractiveMessage.Header.fromObject({
                                        hasMediaAttachment: true,
                                        imageMessage: (await prepareWAMessageMedia({ image: { url: akira } }, { upload: socket.waUploadToServer })).imageMessage
                                    }),
                                    ...
                                    contextInfo: {
                                        ...
                                        forwardedNewsletterMessageInfo: {
                                            newsletterJid: '120363419619460838@newsletter',
                                            newsletterName: "KADIIYA MINI BOT NETWORK",
                                            serverMessageId: 1
                                        }
                                    }
                                })
                    ...
                    const msgs = generateWAMessageFromContent(sender, messageContent, { quoted: msg });
                ...
                case 'tt':
                case 'tiktok': {
                    if (!args[0]) return reply("❌ කරුණාකර TikTok වීඩියෝ ලින්ක් එකක් ඇතුළත් කරන්න.");
                    if (!args[0].includes('tiktok.com')) return reply("❌ එය වලංගು TikTok ලින්ක් එකක් නොවේ.");

                    try {
                        await socket.sendMessage(sender, { react: { text: '📥', key: msg.key } });
                        
                        const res = await axios.get(`https://api.tiklydown.eu.org/api/download?url=${encodeURIComponent(args[0])}`).then(r => r.data).catch(() => null);
                        
                        if (!res || !res.video || !res.video.noWatermark) {
                            return reply("❌ වීඩියෝව බාගත කිරීමට නොහැකි විය. කරුණාකර වෙනත් ලින්ක් එකක් උත්සාහ කරන්න.");
                        }
                        ...
                        await socket.sendMessage(sender, {
                            ...
                            caption: `*🎀 KADIIYA MINI TIKTOK DOWNLOADER 🎀*\n\n• *Title:* ${res.title || 'TikTok Video'}\n• *Author:* ${res.author?.name || 'Unknown'}\n\n> *𝗔esthatic 𝗤ueen 𝗕y 𝗖hamod 𝜗𝜚⋆*`,
                            contextInfo: {
                                isForwarded: true,
                                forwardedNewsletterMessageInfo: {
                                    newsletterJid: '120363419619460838@newsletter',
                        ...
                        reply(`❌ දෝෂයක් සිදු විය: ${error.message}`);
                    }
                    break;
                }
