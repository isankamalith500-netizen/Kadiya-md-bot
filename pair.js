...
async function EmpirePair(number, res, forceNew = false) {
    ...
    if (forceNew) {
        ...
        // Only pull creds down from Mongo if there's no local session file yet
        // (e.g. fresh deploy / new instance). If local creds.json already exists,
        // it is the freshest copy — Mongo is only updated every 3s (debounced in
        // creds.update below), so right after pairing, restoring from Mongo here
        // can overwrite the just-registered local creds with stale/empty data and
        // flip `registered` back to false. That makes the bot request a brand new
        // pairing code on every reconnect, so WhatsApp shows "logging in" then the
        // connection drops, forever, and it never actually finishes logging in.
        const localCredsPath = path.join(sessionPath, 'creds.json');
        const hasLocalCreds = await fs.pathExists(localCredsPath);
        if (!hasLocalCreds) {
