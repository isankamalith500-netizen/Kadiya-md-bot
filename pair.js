...
const SessionSchema = new mongoose.Schema({
    ...
    // Full contents of every OTHER file useMultiFileAuthState writes to the
    // session folder (pre-keys, sender-keys, session records, app-state sync
    // keys, etc.) as { filename: fileContentString }. Without this, only
    // creds.json survives a restart on hosts with ephemeral disk (Railway) —
    // the signal key material is gone, so the restored session looks valid
    // (registered:true) but can't actually decrypt/encrypt and silently
    // stops working after a redeploy/restart.
    keys: {
        type: Object,
        default: {}
    },
...
// ───────── MEMORY WATCHDOG ─────────
// Railway (and most container hosts) OOM-kill the process with ZERO log
// output when it exceeds the plan's memory limit — the process just stops,
// which matches "bot goes offline with no error." Logging RSS every minute
// makes that visible in Railway's log tab (look for the number climbing
// steadily right before a disconnect). The fix once you see it climbing is
// either raising the Railway memory limit or lowering the max concurrent
// sessions (see the activeSockets.size >= 77 check further down).
const MEMORY_WARN_MB = 400;
setInterval(() => {
    const mem = process.memoryUsage();
    const rssMb = (mem.rss / 1024 / 1024).toFixed(1);
    const heapMb = (mem.heapUsed / 1024 / 1024).toFixed(1);
    console.log(`🧠 Memory | RSS: ${rssMb}MB | Heap: ${heapMb}MB | Active sessions: ${activeSockets.size}`);
    if (mem.rss / 1024 / 1024 > MEMORY_WARN_MB) {
        console.warn(`⚠️ Memory usage high (${rssMb}MB). If this keeps climbing toward the Railway plan limit, the process will be OOM-killed with no error log. Consider lowering the max concurrent sessions or upgrading the Railway memory limit.`);
    }
}, 60 * 1000);
...
// Tracks consecutive reconnect attempts per number so a permanently-broken
// session (bad network, WA server issue, etc.) doesn't reconnect-loop forever
// and burn memory/CPU. Resets to 0 as soon as the socket opens successfully.
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        ...
        console.warn(`[${id}] Connection closed | code: ${statusCode} | reason: ${errorMsg}`);

        // 401 = logged out / session invalidated on the WhatsApp side.
        // Reconnecting is pointless here — the creds are dead, so wipe them
        // and require the user to pair again.
        if (statusCode === 401) {
            console.warn(`[${id}] Logged out (401). Wiping session — re-pair required.`);
        ...
        // Every other close reason (515 restart-required, connection lost,
        // timed out, stream error, no reason at all, etc.) is treated as
        ...
        if (attempts > MAX_RECONNECT_ATTEMPTS) {
            console.error(`[${id}] Exceeded ${MAX_RECONNECT_ATTEMPTS} reconnect attempts. Giving up until next manual/boot retry.`);
            await destroySocket(id);
            reconnecting = false;
            return;
        }
        ...
        // doesn't hammer WhatsApp's servers or the event loop.
        const backoff = Math.min(2000 * attempts, 30000);
        console.warn(`[${id}] Reconnecting in ${backoff}ms (attempt ${attempts}/${MAX_RECONNECT_ATTEMPTS})`);
        await delay(backoff);
        await destroySocket(id);
        ...
        try {
            ...
            console.error(`[${id}] Reconnect attempt failed:`, e.message);
        }
...
async function saveSession(number, creds) {
    try {
        ...
        // Back up EVERY file in the session folder (pre-keys, sender-keys,
        // session-*.json, app-state-sync-*.json, not just creds.json) so a
        // restart on ephemeral disk (Railway) can fully restore the signal
        // key material, not just the creds. Without this, restored sessions
        // look "registered" but can't actually en/decrypt messages.
        const files = await fs.readdir(sessionPath);
        const keys = {};
        for (const file of files) {
            if (file === 'creds.json') continue;
            try {
                keys[file] = await fs.readFile(path.join(sessionPath, file), 'utf8');
            } catch (e) {
                // Skip files that vanish mid-read (Baileys can delete/rewrite
                // key files concurrently) — not fatal, they'll be re-synced.
            }
        }
        ...
        console.log(`Saved session for ${sanitizedNumber} to MongoDB (${Object.keys(keys).length} key files), local storage, and numbers.json`);
    } catch (error) {
...
async function restoreSession(number) {
    try {
        ...
        // Restore every backed-up key file (pre-keys, sender-keys, session
        // records, app-state-sync keys) alongside creds.json. Without this,
        // only creds.json comes back after a restart on ephemeral disk —
        // the session looks "registered" but Baileys has no signal key
        // material to actually decrypt/encrypt with, so it silently breaks.
        ...
        for (const [filename, content] of Object.entries(keys)) {
            try {
                ...
                console.error(`Failed to restore key file ${filename} for ${sanitizedNumber}:`, e.message);
            }
        }

        console.log(`Restored session for ${sanitizedNumber} from MongoDB (${restoredKeyCount} key files)`);
...
async function EmpirePair(number, res, forceNew = false) {
    console.log(`Initiating pairing/reconnect for ${number} (forceNew=${forceNew})`);
    const sanitizedNumber = number.replace(/[^0-9]/g, '');
    const sessionPath = path.join(SESSION_BASE_PATH, `session_${sanitizedNumber}`);
    ...
    if (forceNew) {
        // User is explicitly asking for a brand-new pairing code (via /code).
        // Wipe any stale/previous session (Mongo + local files) first, otherwise
        // useMultiFileAuthState will load old creds, socket.authState.creds.registered
        // will be true, the requestPairingCode block below gets skipped entirely,
        // and the HTTP request just hangs with no response ever sent.
        await deleteSession(sanitizedNumber);
    } else {
        // Decide whether the local creds.json is trustworthy enough to use as-is.
        // It's trustworthy only if it exists, parses, and is actually a completed
        // registration (registered:true + me.id present).
        //
        // - If it's trustworthy: leave it alone. Mongo is only updated every 3s
        //   (debounced in creds.update below), so right after pairing, blindly
        //   restoring from Mongo here can overwrite the just-registered local
        //   creds with stale/empty data and flip `registered` back to false —
        //   WhatsApp shows "logging in" then the connection drops, forever.
        // - If it's missing/corrupt/incomplete (fresh deploy with no disk, a
        //   crash mid-write, etc.): fall back to the Mongo backup so already
        //   -connected numbers can self-heal instead of getting stuck unable
        //   to reconnect ("Service Unavailable").
        const localCredsPath = path.join(sessionPath, 'creds.json');
        let localCredsAreValid = false;
        try {
            const raw = await fs.readFile(localCredsPath, 'utf8');
            const parsed = JSON.parse(raw);
            localCredsAreValid = !!(parsed && parsed.registered && parsed.me && parsed.me.id);
        } catch {
            localCredsAreValid = false;
        }

        if (!localCredsAreValid) {
    ...
    try {
        const socket = makeWASocket({
            ...
            auth: {
                creds: state.creds,
                // Without this cache wrapper, every signal-key lookup during
                // message en/decryption hits disk directly. Under any real
                // message volume this gets slow enough that Baileys' internal
                // retry/timeout logic starts treating the socket as dead,
                // which shows up exactly as "works for a few minutes, then
                // goes quiet" with no clear error.
                keys: makeCacheableSignalKeyStore(state.keys, pino({ level: "silent" }))
            },
            logger: pino({ level: "silent" }),
            browser: Browsers.macOS("Safari"), // Pairing code requires the 'Name (OS)' format from the Browsers helper — a raw custom array here causes the "logging in..." freeze on the phone.
            printQRInTerminal: false,
            syncFullHistory: false,      // පරණ මැසේජ් ඔක්කොම ඩවුන්ලෝඩ් වෙන එක නවත්තනවා
            markOnlineOnConnect: false,  // ලොග් වෙද්දී බර අඩු කරනවා
            // Cloud hosts (Railway included) often have NAT/proxy layers that
            // silently drop idle TCP connections well before Baileys' default
            // keepalive would notice. A shorter keepalive interval plus a
            // generous connect timeout makes the socket detect and recover
            // from a dead connection instead of just going silent.
...
// ───────────────────────────────────────────────────
            // NOTE: connection === 'close' handling (including the 401
            // logout case) is fully owned by setupAutoRestart(), which is
            // registered on this same socket. Do not duplicate cleanup logic
            // here — two independent 'close' handlers both racing to call
            // destroySocket/deleteSession was causing inconsistent state.
        });

    } catch (error) {
        socketCreationTime.delete(sanitizedNumber);
        console.error(`❌ EmpirePair failed for ${sanitizedNumber}:`, error?.message || error);
        if (!res.headersSent) {
            res.status(503).send({ error: 'Service Unavailable', reason: error?.message || 'unknown_error' });
        }
    }
}
...
// ════════════ DELUSER (HIDDEN, DEV-ONLY) ════════════
// Removes a bot user's session completely: disconnects the live socket
// (if connected), and wipes their creds from MongoDB, local disk, and
// numbers.json. After this, that number is free to pair/link again from
// scratch. This is intentionally NOT added to any menu/CATEGORY_MAP list,
// and it only works for the literal developer number below — not the
// generic isOwner/isbot check — so no other linked number can ever use it,
// even on their own bot session.
case 'deluser':
case 'removeuser':
case 'unlinkuser': {
    if (senderNumber !== config.OWNER_NUMBER) {
        // Silently ignore for everyone else — command doesn't "exist" for them.
        return;
    }

    const targetRaw = args[0];
    if (!targetRaw) {
        return reply(`*❗ Provide a number!*\n📋 Example: ${sessionConfig.PREFIX || '.'}deluser 947XXXXXXXX`);
    }
    ...
    try {
        ...
        // Disconnect the live socket for that number first, if it's currently connected.
        const targetData = activeSockets.get(targetNumber);
        if (targetData) {
            try { targetData.socket?.ws?.close?.(); } catch (_) {}
            try { targetData.socket?.end?.(); } catch (_) {}
            activeSockets.delete(targetNumber);
            socketCreationTime.delete(targetNumber);
            socketHandlersMap.delete(targetNumber);
        }

        // Wipe creds from MongoDB + local session folder + numbers.json.
        await deleteSession(targetNumber);

        await reply(
            `*✅ User Removed!*\n\n` +
            `📞 Number: ${targetNumber}\n` +
            `🗑️ Session deleted from database & disk.\n` +
            `🔌 Disconnected (if it was active).\n\n` +
            `_That number can link a new device again whenever needed._`
        );
        await socket.sendMessage(sender, { react: { text: '✅', key: msg.key } });
    } catch (err) {
        console.error('deluser error:', err);
        await reply(`❌ *Failed to remove user!* ${err.message}`);
    }
    break;
}
...
router.get('/', async (req, res) => {
    ...
    // NOTE: was hardcoded 77. Each Baileys socket realistically costs tens of
    // MB of RSS, and on Railway's smaller plans (512MB–1GB) that many
    // concurrent sessions is very likely what's pushing the process over the
    // memory limit and getting it silently OOM-killed (watch the 🧠 Memory
    // log line above as sessions grow to see if RSS approaches the plan
    // limit). Raise this back up via the MAX_CONCURRENT_SESSIONS env var
    // only after confirming available memory comfortably supports it.
    const MAX_CONCURRENT_SESSIONS = parseInt(process.env.MAX_CONCURRENT_SESSIONS || '25', 10);
    if (activeSockets.size >= MAX_CONCURRENT_SESSIONS) {
...
// Disconnects every currently connected number, wipes their creds from Mongo
// AND local disk, and clears numbers.json — so every number has to pair
// (request a new code) from scratch again. Destructive and irreversible,
// so it requires the exact confirm phrase below as a query param, e.g.:
//   GET /clear-all?confirm=yes-clear-everything
async function clearAllSessions() {
    const clearedNumbers = Array.from(activeSockets.keys());

    // Close every live socket first so WhatsApp sees a clean disconnect
    // instead of the process just dropping the TCP connection.
    for (const [num, data] of activeSockets) {
        try { data.socket?.ws?.close?.(); } catch {}
        try { data.socket?.end?.(); } catch {}
    }
    activeSockets.clear();
    socketCreationTime.clear();
    socketHandlersMap.clear();

    // Wipe every session document in Mongo.
    let mongoDeletedCount = 0;
    try {
        const result = await Session.deleteMany({});
        mongoDeletedCount = result?.deletedCount || 0;
    } catch (e) {
        console.error('clearAllSessions: Mongo deleteMany failed:', e.message);
    }

    // Wipe the local session folder entirely and recreate it empty.
    try {
        await fs.emptyDir(SESSION_BASE_PATH);
    } catch (e) {
        console.error('clearAllSessions: failed to empty session dir:', e.message);
    }

    // Reset numbers.json so auto-reconnect-on-startup doesn't try to revive
    // anything we just wiped.
    try {
        await fs.writeFile(NUMBER_LIST_PATH, JSON.stringify([], null, 2));
    } catch (e) {
        console.error('clearAllSessions: failed to reset numbers.json:', e.message);
    }

    return { clearedNumbers, mongoDeletedCount };
}
...
router.get('/clear-all', async (req, res) => {
    ...
    if (confirm !== 'yes-clear-everything') {
        return res.status(400).send({
            ...
            message: 'This wipes every connected number and cannot be undone. Resend the request with ?confirm=yes-clear-everything to proceed.'
        });
    }

    try {
        const { clearedNumbers, mongoDeletedCount } = await clearAllSessions();
        console.log(`🧹 Cleared all sessions. Numbers disconnected: ${clearedNumbers.length}, Mongo docs deleted: ${mongoDeletedCount}`);
        res.status(200).send({
            status: 'cleared',
            disconnectedNumbers: clearedNumbers,
            mongoDocsDeleted: mongoDeletedCount,
            message: 'All numbers have been disconnected and their sessions wiped. Everyone needs to pair with a new code now.'
        });
    } catch (error) {
        console.error('clear-all failed:', error);
        res.status(500).send({ error: 'Failed to clear sessions', reason: error.message });
    }
});
...
process.on('uncaughtException', (err) => {
    // On Railway there is no pm2 process to restart — `exec('pm2 restart ...')`
    // silently fails (pm2 isn't installed there), so the process was crashing
    // with ZERO log output, which is exactly the "bot just disappears" symptom.
    // Just log it fully and keep the process alive; Railway's own restart
    // policy will only kick in if the process actually exits, which we no
    // longer do here for a single bad exception.
